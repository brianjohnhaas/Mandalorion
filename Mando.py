...
parser.add_argument(
    "--defineIsoformsDelayTime",
    type=int,
    default=1200,
    help="delay time set in defineIsoforms.py",
)
...
delayTime = args.defineIsoformsDelayTime
...
if "P" in Modules:
    ...
    if input:
        ...
        SpliceDefineConsensus.get_loci(
            clean_sorted_psl_file, temp_path + "/tmp_SS", minimum_reads
        )
    else:
        print(
...
if "D" in Modules:
    ...
    if everything_present:
        os.system(
            "python3 %s/defineIsoforms.py -i %s -p %s -c %s -g %s -w %s -m %s -W %s -n %s -j %s -u %s -d %s -a %s --delaytime %i"
            % (
                MandoPath,
...
if "F" in Modules:
    ...
    if everything_present:
        ...
        os.system(
            "sort -T %s -k 9,9 -k 14,14 -k 16,17n %s > %s"
            % (
                temp_path,
        ...
        print(
            "\tgrouping isoforms, assigning them to genes (if annotation is provided), and creating gtf"
        )
        os.system(
